...
uint8 is_ring()
{
    ...
    if(ad[1] > (500 + (car_info.angle - 20) * (car_info.angle<20 ? 13 : 6)) && ad[0] > (530 + (car_info.angle - 20) * (car_info.angle<20 ? 13 : 6))) //TODO: 由于电感原因，右环条件可能需要进一步调节
    {
...
{   
    ...
    if(ring_dir == RIGHT)
    {
        if(car_info.distance > 210)  // 移动超过1cm
            return 1;
        else
            return 0;
    }
    ...
    {
        if(car_info.distance > 270)  // 移动超过27cm
            return 1;
        else
            return 0;
    }
